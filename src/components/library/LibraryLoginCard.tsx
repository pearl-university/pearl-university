...
import { validateLibraryCredentials } from '../../utils/auth'
import { useAuth } from '../../context/AuthContext'


const libraryLoginValidationSchema = Yup.object().shape({
...
export const LibraryLoginCard: FC = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: libraryLoginValidationSchema,
    onSubmit: async (values, { setSubmitting, setFieldError }) => {
      setAuthStatus('idle')
      showLoader('Authenticating with Pearl e-Library gateway...')

      // 1.8s simulated authentication delay
      await new Promise((resolve) => setTimeout(resolve, 1800))

      const result = validateLibraryCredentials(values.studentId, values.password)
      ...
      if (result.success && result.user) {
        ...
        alert.success(
          ...
          `Welcome back, ${result.user.name}. Entering digital library.`
        )
        setTimeout(() => {
        ...
        hideLoader()
        setSubmitting(false)
        const errorMsg = result.error || 'Invalid Student ID or Password combination.'
  ...
  return (
    <section className="w-full min-h-[90vh] flex items-center justify-center py-10 sm:py-16 md:py-20 px-4 sm:px-6 md:px-8 lg:px-12 bg-white">
      <div className="w-full mx-auto">
        {/* Main Lavender Auth Card matching screenshot s3.png */}
        <motion.div
          variants={fadeInUp}
          initial="hidden"
          animate="visible"
        ...
        >
          ...
          <div className="lg:col-span-5 flex flex-col justify-center max-w-md mx-auto lg:mx-0 w-full">
            ...
            <p className="text-xs sm:text-sm text-[#200441]/75 font-normal mt-3 mb-8 sm:mb-10">
              Access the Pearl University Digital Library Catalogue, research repositories, e-journals, and databases.
            </p>
            ...
            <form onSubmit={formik.handleSubmit} noValidate className="flex flex-col gap-4 sm:gap-5 w-full">
              ...
              <div className="flex flex-col">
                <div className="relative flex items-center w-full">
                  <div className="absolute left-5 text-[#200441] pointer-events-none">
                    <FiMail className={`w-5 h-5 transition-opacity ${isStudentIdError ? 'opacity-100 text-red-600' : 'opacity-80'}`} />
                  </div>
                  <input
                    type="text"
                    id="studentId"
                    ...
                    aria-label="Student ID or Institutional Email"
                    aria-invalid={isStudentIdError}
                    aria-describedby={isStudentIdError ? 'studentId-error' : undefined}
                    className={`w-full pl-13 pr-6 py-3.5 sm:py-4 bg-transparent rounded-full text-base sm:text-[17px] text-[#200441] placeholder:text-[#200441]/50 focus:outline-none transition-all font-normal ${isStudentIdError
              ...
              <div className="flex flex-col">
                <div className="relative flex items-center w-full">
                  <div className="absolute left-5 text-[#200441] pointer-events-none">
                    <FiLock className={`w-5 h-5 transition-opacity ${isPasswordError ? 'opacity-100 text-red-600' : 'opacity-80'}`} />
                  </div>
                  <input
                    ...
                    className={`w-full pl-13 pr-13 py-3.5 sm:py-4 bg-transparent rounded-full text-base sm:text-[17px] text-[#200441] placeholder:text-[#200441]/50 focus:outline-none transition-all font-normal ${isPasswordError
                        ? 'border border-red-500 bg-red-50/20 focus:border-red-600 focus:ring-2 focus:ring-red-300/40'
                        : 'border border-[#200441]/35 focus:border-[#200441] focus:ring-2 focus:ring-[#200441]/20'
                      }`}
                  />
              ...
              <AnimatePresence mode="wait">
                {authStatus === 'success' && (
                  ...
                  >
                    ...
                    <span>Access Granted! Welcome to Pearl University E-Library.</span>
                  </motion.div>
                )}
              </AnimatePresence>
          ...
          {/* Right Column: Library Photo with Rounded Corners */}
          <div className="lg:col-span-7 w-full h-full flex items-center justify-center">
            <div className="relative w-full aspect-[4/3] sm:aspect-[14/11] lg:aspect-[4/5] rounded-[24px] sm:rounded-[32px] overflow-hidden shadow-2xl shadow-black/10 bg-gray-200 group">
              <img
                src={libraryAuthImg}
                alt="Pearl University students smiling and collaborating in the modern library"
